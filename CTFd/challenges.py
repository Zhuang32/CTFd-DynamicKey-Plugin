...
def chals():
    ...
    if utils.user_can_view_challenges() and (utils.ctf_started() or utils.is_admin()):
        ...
        for x in chals:
            ...
            files = [str(f.location) for f in Files.query.filter_by(chal=x.id, dynamic=False).all()]
            dfiles = [str(f.location) for f in Files.query.filter_by(chal=x.id, dynamic=True).all()]
    ...
    if (utils.authed() and utils.is_verified() and (utils.ctf_started() or utils.view_after_ctf())) or utils.is_admin():
        ...
        if not solves:
            ...
            if status:  # The challenge plugin says the input is right
                ...
                if isinstance(message, int):  # This means someone submitted others' flag
                    if utils.ctftime() or utils.is_admin():
                        chal_class.penalty(team1=team, team2=Teams.query.filter_by(id=message).first(), chal=chal, request=request)
                    logger.info("[{0}] {1} submitted {2} with kpm {3} [CHEAT]".format(*data))
