...
@admin_statistics.route('/admin/cheat_penalties', defaults={'page': '1'}, methods=['GET'])
@admin_statistics.route('/admin/cheat_penalties/<int:page>', methods=['GET'])
@admins_only
def admin_cheat_penalty(page):
    page = abs(int(page))
    results_per_page = 50
    page_start = results_per_page * (page - 1)
    page_end = results_per_page * (page - 1) + results_per_page

    penalties = CheatPenalty.query.add_columns(CheatPenalty.id, CheatPenalty.chalid, CheatPenalty.teamid, CheatPenalty.date, CheatPenalty.flag, 
                                      CheatPenalty.penalty, Challenges.name.label('chal_name'), Teams.name.label('team_name')) \
        .join(Challenges) \
        .join(Teams) \
        .order_by(CheatPenalty.date.desc()) \
        .slice(page_start, page_end) \
        .all()

    penalty_count = db.session.query(db.func.count(CheatPenalty.id)).first()[0]
    pages = int(penalty_count / results_per_page) + (penalty_count % results_per_page > 0)

    return render_template('admin/cheat_penalties.html', penalties=penalties, pages=pages, curr_page=page)
