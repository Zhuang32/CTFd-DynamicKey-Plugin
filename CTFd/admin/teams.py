...
from CTFd.models import db, Teams, Solves, Awards, Unlocks, Challenges, WrongKeys, Keys, Tags, Files, Tracking, Pages, Config, DatabaseError, CheatPenalty
...
@admin_teams.route('/admin/cheat_penalties/<int:keyid>/<int:team_id>/<int:value>/delete', methods=['POST'])
@admins_only
def delete_cheat_penalty(keyid,team_id,value):
    cp = CheatPenalty.query.filter_by(id=keyid).first_or_404()
    db.session.delete(cp)
    award = Awards.query.filter_by(teamid=team_id,value=-value).first_or_404()
