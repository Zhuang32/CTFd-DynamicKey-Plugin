...
from CTFd.models import db, Teams, Solves, Awards, Files, Pages, Keys
from CTFd.utils import cache, markdown
from CTFd import utils
from CTFd.plugins.keys.aes import prpcrypt
...
def profile():
    if utils.authed():
        if request.method == "POST":
            ...
            if len(errors) > 0:
                return render_template('profile.html', name=name, email=email, token=user.token, website=website,
            ...
            return render_template('profile.html', name=name, email=email, token=token, website=website, affiliation=affiliation,
...
def file_handler(path):
    ...
    if not os.path.exists(teamfile_path):
        os.system("python %s %s %s %s"%(safe_join(upload_folder, f.location),filedir_path,session['token'],app.root_path))
